...
def process_data(rdd, time_period):
    ...
    def valid_record(rdd):
        try:
            if rdd[1].strip() == "":
                raise Exception("missing country")

            if rdd[3].strip() == "" and rdd[4].strip() == "":
    ...
    data_info = valid_data.map(get_event_code) \
                           ...
                           .map(lambda x: (country_names[x[0]], x[1], event_names[x[2]] +"-"+ event_cause[x[3]], x[4], x[5], x[6])) \
    # Add keys to data
...
monthly_aggr_data = process_data(data, "monthly")
monthly_aggr_data.take(5)
